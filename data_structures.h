...
void pushQueue(struct Queue* q, struct PCB new_proc) {
...
struct PCB popQueue(struct Queue* q) {
    if (q->front == NULL) {
        printf("Error: Attempt to pop from an empty queue\n");
        struct PCB empty_proc;
        memset(&empty_proc, -2, sizeof(struct PCB)); // Return an empty PCB with default values
        return empty_proc;
    }

    struct QueueNode* temp = q->front;
    struct PCB proc = q->front->proc;
